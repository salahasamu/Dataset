...
根据截图显示，数据集被分卷压缩（1-20, 21-40, 41-52），这意味着该子集总共包含 **52** 个不同的 ROI（细胞样本）。解压后，内部以 **Cell (细胞)** 为单位进行组织。

**目录树结构如下：**

Plaintext

BioTISR_Microtubules_3D_XX-XX/ # 数据集分卷根目录 (如 1-20)
...
└── Cell_052/ # 总计 52 个 ROI (根据分卷文件名推断)
...
- **样本数量**: 包含 **52** 个不同的 ROI（根据文件名 41-52 确认）。
