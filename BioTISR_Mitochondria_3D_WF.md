# **BioTISR Dataset: 3D Mitochondria (TOMM20) Subset Structure**

## **1\. 简介 (Introduction)**

该子数据集是 **BioTISR** (Biological Time-lapse Image Super-Resolution) 库的 **3D Mitochondria (线粒体/线粒体外膜)** 部分。数据通过 **3D-SIM** (三维结构光照明荧光显微镜) 模式采集。该数据集旨在提供高质量的时序生物超分辨率 (SR) 图像，用于研究线粒体的复杂形变（如分裂与融合）以及 3D 体数据的重建算法。

## **2\. 文件夹结构详情 (Folder Structure Details)**

根据截图显示，压缩包解压后的根目录名为 BioTISR_TOMM20_3D（TOMM20 是线粒体外膜的常用标记物），内部以 **Cell (细胞)** 为单位进行组织。

**目录树结构如下：**

BioTISR_TOMM20_3D/ # 数据集根目录 (3D 线粒体外膜数据)
...
- **特性**: 线粒体外膜呈现双层膜结构，在 3D-SIM 下表现为中空的管状或网状结构，真值图像已解算出这些拓扑细节。

## **4\. 关键技术规格 (Technical Specifications)**

- **生物结构**: Outer Mitochondrial Membrane (线粒体外膜)，标记为 TOMM20。
